...
for lat_i = 23:23%1:50
    ...
    for ai = 1:length(ablated_cells)
        if ablated_cells(ai)~=119, continue; end
        close all
        load([stage2_fold,'/lat(', num2str(lat_i),')_step(',num2str(s2tf),').mat'],'g');
        timer = 0;
        ...
        for t =1:tfinal
            save(['ablated_cells/ablation-lat(',num2str(lat_i),')-cell(',num2str(ablated_cells(ai)),')',num2str(t-1) ,'_zoom.mat'],'g');
        ...
        close(vid)
        save(['ablated_cells/ablation-lat(',num2str(lat_i),')-cell(',num2str(ablated_cells(ai)),')', num2str(t) ,'_zoom.mat'],'g');
    end
