...
pars_name = ["area","tension","contractility","","repulsion","","shear"];
my_ps = [6 ; 0.3; 0.15; 0; 1; 0.5; 0.4];

for pars = 1:length(pars_name)
    if pars_name(pars) == "", continue; end
    savefold = strcat('stage1_',pars_name(pars));
    mp = my_ps(pars);
    ps = (0.5:0.25:1.5)*mp;
    ...
    for pind = 1:length(ps)
        ...
        for lat_i=1:20
            ...
            g = gs{lat_i};
            save(strcat(savefold, '/lat(', num2str(lat_i),')_p-', pars_name(pars),'=',num2str(ps(pind)),'_final.mat'),'g');
        end
