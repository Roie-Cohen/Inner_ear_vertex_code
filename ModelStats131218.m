...
l27 = []; % lattices with 27 HCs in the end of simulation
for i=1:30
    load(['stage', num2str(stg),'_complete-',datestr,'\lat(', num2str(i),')_step(', num2str(ts-1),').mat'],'g');
    if sum(g.populations == 3) == 27
        l27 = [l27 i];
    end
end
...
for t=0:ts-1
    ...
    for i=1:30
        ...
        load(['stage', num2str(stg),'_complete-',datestr,'\lat(', num2str(i),')_step(', num2str(t),').mat'],'g');
        % only 21 oHCs
        if ~ismember(i, l27), continue; end
        % differentiate
    ...
    save(['stage', num2str(stg),'_complete-',datestr,'\stats_step(', num2str(t),')',ext,'.mat'],'gof','non','ar');
    non_t(t+1) = mean(non);
...
close(wb);
save(['stage', num2str(stg),'_complete-',datestr,'\stats_all',ext,'.mat'],'gof_t','non_t','ar_t');
